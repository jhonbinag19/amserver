...
const IntegrationTools = ({ connectedTools = [], onToolSync, onToolDisconnect }) => {
  ...
  const handleSyncTool = async (toolId) => {
    try {
      ...
      await axios.post(`/api/integrations/tools/${toolId}/sync`);
  ...
  const handleDisconnectTool = async (toolId) => {
    try {
      await axios.delete(`/api/integrations/tools/${toolId}`);
      message.success('Tool disconnected successfully');
      onToolDisconnect && onToolDisconnect();
    } catch (error) {
  ...
  const handleAddTool = async (values) => {
    try {
      await axios.post('/api/integrations/tools', values);
    ...
    <div className="integration-tools">
      <Card
        title={
          <Space>
            <ApiOutlined />
            <Title level={4} style={{ margin: 0 }}>Integrated Tools</Title>
          </Space>
        }
        extra={
          <Button
